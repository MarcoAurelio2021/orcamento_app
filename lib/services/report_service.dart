...
class ReportService {
  ...
  static String budgetAsText(Budget budget, {CompanyData? company, bool includeCompany = false}) {
    ...
    if (includeCompany && company != null && company.hasAnyData) {
      ...
      if (company.phone.isNotEmpty) buffer.writeln('Telefone: ${company.phone}');
      if (company.email.isNotEmpty) buffer.writeln('E-mail: ${company.email}');
      if (company.address.isNotEmpty) buffer.writeln('Endereço: ${company.address}');
      buffer.writeln('');
    }

    buffer.writeln('ORÇAMENTO ${budget.number}');
    buffer.writeln('Cliente: ${budget.clientName}');
    if (budget.technician.isNotEmpty) buffer.writeln('Técnico: ${budget.technician}');
    if (budget.address.isNotEmpty) buffer.writeln('Endereço: ${budget.address}');
    if (budget.paymentMethod.isNotEmpty) buffer.writeln('Pagamento: ${budget.paymentMethod}');
    buffer.writeln('Data: ${_date.format(budget.createdAt)}');
    buffer.writeln('');

    for (final item in budget.items) {
      final typeLabel = item.valueType == ItemValueType.fixed ? 'Fixo' : 'Percentual';
        ...
        build: (context) => [
          if (includeCompany && company != null && company.hasAnyData)
            pw.Column(
              ...
              children: [
                pw.Text(company.name, style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 16)),
          ...
          pw.Text('Orçamento ${budget.number}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 18)),
