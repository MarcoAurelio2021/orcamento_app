  ...
  static String budgetAsText(
    ...
    for (final item in budget.items) {
      ...
      if (showUnitValues) {
        ...
        buffer.writeln(
          '- ${item.name} | $typeLabel | Qtd: ${item.quantity} | Total: ${_currency.format(total)}',
        );
      }
    }
  ...
  static Future<List<int>> buildPdf(
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) {
          final headers = showUnitValues
              ? const ['Serviço', 'Tipo', 'Valor', 'Qtd', 'Total']
              : const ['Serviço', 'Tipo', 'Qtd', 'Total'];

          final data = budget.items.map((item) {
            final total = item.totalForFixedBase(budget.fixedSubtotal);
